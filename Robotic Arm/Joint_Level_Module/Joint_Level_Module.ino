...
void sendEmergency() {
  ...
  // Calculate CRC and prepare for transmission
  crc.restart();
  for (int i = 0; i <= 9; i++) {
    crc.add(message[i]);
  }
  ...
  // High-priority multi-transmission with reduced delays
  for (int attempt = 0; attempt < 3; attempt++) {  // 3 attempts for emergency
    int retries = 0;
    
    // Shorter bus idle check for emergency (10ms instead of 20ms)
    while (retries < 2 && !isBusIdle(10)) {  // Reduced idle time and retries for urgency
      retries++;
      delay(5);  // Shorter delay between retries
    }
    ...
    if (retries < 2) {
      ...
      delay(2);  // Short gap between transmissions
      
      if (debugFlag) {
        Serial.print("Emergency transmitted - Attempt ");
        Serial.print(attempt + 1);
        Serial.print(" Type: 0x");
        Serial.println(emergencyType, HEX);
      }
    } else if (debugFlag) {
      Serial.println("Bus busy, emergency attempt " + String(attempt + 1) + " skipped");
    }
  }

  Serial.print("EMERGENCY BROADCAST COMPLETE! Type: 0x");
  Serial.print(emergencyType, HEX);
  Serial.print(" at Joint ");
...
void isrLimit1() {
  if (!emergencyFlagActive && !limitSwitch1Emergency) {  // CHANGED THIS LINE
    triggerEmergency(EMERGENCY_LIMIT_SWITCH_1);
  }
}

void isrLimit2() {
  if (!emergencyFlagActive && !limitSwitch2Emergency) {  // CHANGED THIS LINE
...
void checkEmergencyConditions() {
  ...
  if (millis() - lastEmergencyCheck > 20) {
    ...
    if (!emergencyFlagActive) {  // CHANGED THIS LINE
      ...
      if (abs(slippage) > (SLIPPAGE_THRESHOLD * 30)) {
        triggerEmergency(EMERGENCY_SLIPPAGE_CRITICAL);
      }
      // Check for encoder faults
      else if (motorAngle < -1000 || motorAngle > 50000 || gearboxAngle < -1000 || gearboxAngle > 50000) {
...
void checkLimitSwitches() 
{
  ...
  // REMOVE THIS EMERGENCY HANDLING - ISRs handle it
  // The debounced values are used for status reporting only
  if (debugFlag && (debounced_limit1 || debounced_limit2)) {
    Serial.println("Limit Switch Detected (debounced)");
  }
}
