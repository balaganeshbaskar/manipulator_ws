// ============================================================
// PRECISION JOINT CONTROLLER - GEARBOX ENCODER ONLY
// ============================================================
// With settling phase and iterative correction for high accuracy
// Target precision: ±0.2° (±0.5° acceptable)
// ============================================================
...
const float DEGREES_PER_MOTOR_STEP = 1.8;
...
// ============================================================
const float MAX_VELOCITY = 30.0;           // deg/s
const float ACCELERATION = 50.0;           // deg/s²
const float DECEL_SAFETY_FACTOR = 2.5;     // Start decel at 18° from target - was 2.0

// Initial stop threshold
const float DEADZONE = 0.3;                // ±0.5° initial stop threshold

// Precision thresholds for settling phase
const float PRECISION_THRESHOLD = 0.1;     // ±0.2° target accuracy
const float MAX_ACCEPTABLE_ERROR = 0.3;    // Trigger correction above this

// Speed parameters
const float MIN_SPEED_FAR = 2.0;           // When >5° away
const float MIN_SPEED_NEAR = 1.0;          // When 2-5° away  
const float MIN_SPEED_CLOSE = 0.5;         // When <2° away
const float CORRECTION_SPEED = 0.8;        // Fast but safe correction (0.8 deg/s)

// Settling parameters
const unsigned long SETTLING_DURATION = 100;  // Wait 100ms for mechanical settling
const uint8_t MAX_CORRECTION_ATTEMPTS = 2;    // Maximum correction iterations
...
enum ControlState {
  ...
  CRUISING,
  DECELERATING,
  SETTLING,        // NEW: Wait for mechanical settling
  CORRECTING,      // Corrective move if error too large
  HOLDING
};
...
void setup() {
  ...
  Serial.println(F("\n╔═══════════════════════════════════════════════════╗"));
  Serial.println(F("║  PRECISION JOINT CONTROLLER                       ║"));
  Serial.println(F("╠═══════════════════════════════════════════════════╣"));
  Serial.println(F("║  Features:                                        ║"));
  Serial.println(F("║   • Gearbox encoder absolute positioning          ║"));
  Serial.println(F("║   • Trapezoidal motion profile                    ║"));
  Serial.println(F("║   • Settling phase (100ms)                        ║"));
  Serial.println(F("║   • Iterative correction (max 2 attempts)         ║"));
  ...
  Serial.print(F("  Initial deadzone:   ±")); Serial.print(DEADZONE); Serial.println(F("°"));
  Serial.print(F("  Target precision:   ±")); Serial.print(PRECISION_THRESHOLD); Serial.println(F("°"));
  Serial.print(F("  Settling time:      ")); Serial.print(SETTLING_DURATION); Serial.println(F("ms"));
  Serial.print(F("  Correction speed:   ")); Serial.print(CORRECTION_SPEED); Serial.println(F(" deg/s"));
  Serial.print(F("  Max corrections:    ")); Serial.println(MAX_CORRECTION_ATTEMPTS);
  Serial.println();

  Serial.println(F("Commands:"));
  Serial.println(F("  M <angle>  - Move to angle"));
  Serial.println(F("  R <delta>  - Relative move"));
  Serial.println(F("  S          - Stop"));
  Serial.println(F("  Q          - Query position"));
  Serial.println(F("  T          - Status"));
  Serial.println();

  // Wait for first encoder reading
  ...
  switch(cmd_type) {
    ...
    case 'Q': {
      ...
      if (joint1.dataValid) {
        ...
        Serial.println(F("Position: No data"));
      }
      break;
    }
...
void moveToAngle(float target_degrees) {
  ...
  correction_attempts = 0;  // Reset correction counter

  Serial.print(F("\nMoving to "));
  Serial.print(target_degrees, 2);
  Serial.print(F("° ("));
  Serial.print(target_count, 1);
  Serial.println(F(" counts)\n"));
}

void moveRelative(float delta_degrees) {
  float current_angle = countToAngle(current_position_count);
  float new_target = current_angle + delta_degrees;
  moveToAngle(new_target);
}
...
// ============================================================
// CONTROL LOOP - WITH SETTLING & CORRECTION
// ============================================================
void updateControlLoop() {
  // Update current position from gearbox encoder
  current_position_count = (float)joint1.gearboxCount;

  // Calculate error with wraparound
  ...
  float distance_remaining = abs(error_degrees);  // Calculate early!

  // ============================================================
  // STATE: SETTLING - Wait for mechanical settling
  // ============================================================
  if (state == SETTLING) {
    if (millis() - settling_start_time >= SETTLING_DURATION) {
      ...
      if (final_error <= PRECISION_THRESHOLD) {
        ...
        if (correction_attempts >= MAX_CORRECTION_ATTEMPTS) {
          Serial.print(F("⚠ Max corrections reached. Final error: "));
          Serial.print(error_degrees, 2);
          Serial.println(F("°"));
        }
  ...
  if (crossed_target && (state == ACCELERATING || state == CRUISING || state == DECELERATING)) {
    Serial.print(F("⚠ Overshoot! Error: "));
    Serial.print(error_degrees, 2);
    Serial.println(F("° - Entering settling..."));
  ...
  // ============================================================
  // ADAPTIVE DECELERATION SAFETY FACTOR
  // ============================================================
  float safety_factor;
  if (distance_remaining < 30.0) {
    safety_factor = 1.2;
  } else if (distance_remaining < 90.0) {
    safety_factor = 1.8;
  } else {
    safety_factor = 2.5;
  }
  ...
  float decel_start_distance = max_decel_distance * safety_factor;
  ...
  // ============================================================
  // STATE: IDLE - CRITICAL FIX FOR SHORT MOVES
  // ============================================================
  else if (state == IDLE) {
    direction_cw = (position_error_counts > 0.0);
    setDirection(direction_cw);
    
    // Check if this is a short move that won't reach cruise speed
    if (distance_remaining < 15.0) {
      // SHORT MOVE: Start with calculated velocity for triangular profile
      float peak_velocity = sqrt(2.0 * distance_remaining * ACCELERATION * 0.5);
      
      if (peak_velocity > MAX_VELOCITY) {
        peak_velocity = MAX_VELOCITY;
      }
      
      // Start with significant velocity to avoid crawling
      current_velocity = peak_velocity;
      state = DECELERATING;
      
      Serial.print(F("→ Short move: "));
      Serial.print(distance_remaining, 1);
      Serial.print(F("° @ "));
      Serial.print(current_velocity, 1);
      Serial.println(F(" deg/s"));
    } else {
      // Normal move: start acceleration from zero
      current_velocity = 0.0;
      state = ACCELERATING;
    }
  }
  ...
  // ============================================================
    ...
    current_velocity += ACCELERATION * 0.025;
  ...
  // ============================================================
  // STATE: DECELERATING - WITH ADAPTIVE RATES
  // ============================================================
  else if (state == DECELERATING) {
    // Adaptive deceleration rate
    float decel_rate;
    if (distance_remaining > 10.0) {
      decel_rate = ACCELERATION * 0.025;      // 1.25 deg/s per cycle
    } else if (distance_remaining > 5.0) {
      decel_rate = ACCELERATION * 0.04;       // 2.0 deg/s per cycle
    } else {
      decel_rate = ACCELERATION * 0.05;       // 2.5 deg/s per cycle
    }
    
    current_velocity -= decel_rate;

    // Adaptive minimum speeds - CRITICAL FIX
    float min_speed;
    if (distance_remaining > 10.0) {
      min_speed = 3.0;               // 3.0 deg/s when >10° (HIGHER!)
    } else if (distance_remaining > 5.0) {
      min_speed = 2.0;               // 2.0 deg/s when 5-10°
    } else if (distance_remaining > 2.0) {
      min_speed = MIN_SPEED_NEAR;    // 1.0 deg/s when 2-5°
    } else {
      min_speed = MIN_SPEED_CLOSE;   // 0.5 deg/s when <2°
    }
...
void printStatus() {
  ...
  Serial.print(F("  Moving:      ")); Serial.println(move_active ? F("YES") : F("NO"));
  Serial.println();
}
